...
# Combination Modules
class Comb_Position_Embed(nn.Module):
    def __init__(self, in_dim, out_dim):
        super(Comb_Position_Embed, self).__init__()
        self.layer = nn.Sequential(
            nn.Conv2d(in_dim, out_dim, kernel_size=1, bias=False),
            nn.LeakyReLU(0.1),
            nn.Conv2d(out_dim, out_dim, kernel_size=1, bias=False), # , bias=False
            nn.LeakyReLU(0.1))
...
class Combination_Conv(nn.Module):
    def __init__(self, in_dim, out_dim):
        super(Combination_Conv, self).__init__()
        self.layer = nn.Sequential(
...
class Combination_Heads(nn.Module):
    def __init__(self, cfg, in_dim, red_dim, out_dim):
        super(Combination_Heads, self).__init__()
        self.cfg = cfg
        self.layer = nn.Sequential(
            nn.Linear(in_dim, red_dim),
            nn.ReLU(),
            nn.Linear(red_dim, red_dim),
            nn.ReLU(),
            nn.Linear(red_dim, out_dim))
